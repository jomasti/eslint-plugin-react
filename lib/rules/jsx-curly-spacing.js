...
const SPACING_VALUES = [SPACING.always, SPACING.never, SPACING.consistent];
...
module.exports = {
  ...
  create: function(context) {
    ...
    * @returns {void}
    */
    function reportBeginningNewline(node, token) {
      context.report({
        node: node,
        loc: token.loc.start,
        message: `There should be a newline after '${token.value}'`,
        fix: function(fixer) {
          const nextToken = sourceCode.getTokenAfter(token);
    ...
    * @returns {void}
    */
    function reportEndingNewline(node, token) {
      context.report({
        node: node,
        loc: token.loc.start,
    ...
    */
    ...
    function validateBraceSpacing(node) {
      ...
      } else if (spacing === SPACING.consistent) {
        if (isMultiline(first, second) && !isMultiline(penultimate, last)) {
          reportEndingNewline(node, last);
        }
        if (!isMultiline(first, second)) {
          if (isMultiline(penultimate, last)) {
            reportBeginningNewline(node, first);
          }
          if (sourceCode.isSpaceBetweenTokens(first, second) && !sourceCode.isSpaceBetweenTokens(penultimate, last)) {
            reportRequiredEndingSpace(node, last);
          }
          if (!sourceCode.isSpaceBetweenTokens(first, second) && sourceCode.isSpaceBetweenTokens(penultimate, last)) {
            reportRequiredBeginningSpace(node, first);
