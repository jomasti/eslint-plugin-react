...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function checkValidPropType(node) {
      if (node && node.name && !PROP_TYPES.some(propTypeName => propTypeName === node.name)) {
        context.report({
          node: node,
    ...
    function checkValidProp(node) {
      if (node && node.type === 'MemberExpression' && node.object.type === 'MemberExpression') {
        ...
        if (callee.type === 'MemberExpression' && callee.property.name === 'shape' && node.arguments && node.arguments.length) {
